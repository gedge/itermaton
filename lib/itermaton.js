...
function inheritFrom(parentNode, childNode) {
	...
	for (var i in keys) {
		...
		if (key == "init_commands" || key == "late_init_commands") {
			...
			console.log("inherit " + key + " '" + parentNode[key] + "'")
			for (var cmdN in parentNode[key]) {
				childNode[key] = childNode[key].concat(parentNode[key][cmdN])
			}
...
function run(argv) {
	...
	for (var orderIdx = 0; orderIdx < order.length; orderIdx++) {
		...
		if (orderIdx > 0) {
			...
			console.log("Norm " + normalisedOrder + "  Sleep " + sleepy + "  lastOrder " + lastOrder);
			delay(opts.closeWindows ? 2 : sleepy);	// sleep
			...
			for (var t = tFirst; t != tStop; t+=tIncr) {
				...
				if (opts.createWindows && orderIdx == 0) {
					...
					for (var tabNum = 0; tabNum < win.tabs.length; tabNum++) {
						...
						console.log("230 [" + normalisedOrder + "] w " + w + " tab " + t + "  want "+tabName + "  got "+tabNameVal);
				...
				for (var p = pFirst; p != pStop; p+=pIncr) {
					console.log("283 [" + normalisedOrder + "] w " + w + "  tab " + t + "  p2 " + p + "  sess " + sessionIndex + " real opts: " + JSON.stringify(panesList[p]));
					var paneName = panesList[p].name;
					if (paneName === undefined) {
						console.log("285 Cannot find name for pane with index: " + t + " within window: " + paneName + " in tab: " + tabName)
						return 2
					}
					...
					if (opts.createWindows && orderIdx == 0) {
						...
						for (sessionIndex = sessFirst; sessionIndex != sessStop; sessionIndex+=sessIncr) {
							console.log(w+" "+t+" "+p+ " top="+sessionIndex + " len="+tab.sessions.length+"  "+paneName)
							// race condition xref_close_race
							var sess = tab.sessions[sessionIndex]
							if (sess===undefined) {
								console.log(w+" "+t+" "+p+" sess="+sessionIndex + " undef sess for " + paneName)
							}
							var sessNameVal = getVar(sess, "sessName")
							if (sessNameVal === undefined){
								console.log(w+" "+t+" "+p+" sess="+sessionIndex + " undef NAME for " + paneName)
							}
							if (sessNameVal == paneName) {
								gotPane = true
								break
							}
							console.log(w+" "+t+" "+p+" sess="+sessionIndex + "... " + paneName + " != " + sessNameVal)
						}
						console.log(w+" "+t+" "+p+ " post")
						if (!gotPane) {
							console.log("Cannot find pane " + paneName)
							return 2
						}
					}
					console.log(w+" "+t+" "+p+ " l=345")

					var realOrder = opts.defOrder + panesList[p].priority + 1
					console.log(w+" "+t+" "+p+ " l=348")
					...
					console.log(w+" "+t+" "+p+ " l=375")

					if (opts.closeWindows && order[orderIdx] == 0) {
						sendKeys(win, t, sessionIndex, "\004");
						delay(1) // sleep to allow window to close before moving to next (race condition xref_close_race)
					}
					console.log(w+" "+t+" "+p+ " l=380")

				}
				console.log(w+" "+t+" "+p+ " l=383")
