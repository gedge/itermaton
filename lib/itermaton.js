...
function inheritFrom(parentNode, childNode) {
	var keys = ['start_command', 'stop_command', 'stop_is_sigint', 'env', 'debug']
	for (var i in keys) {
		var key = keys[i]
		if (parentNode[key] === undefined) {
			...
			for (var envVar in parentNode[key]) {
				console.log("inherit env '" + envVar + "'")
				childNode[key][envVar] = parentNode[key][envVar]
			}
		}
...
function run(argv) {
	...
	for (var orderIdx = 0; orderIdx < order.length; orderIdx++) {
		...
		normalisedOrder = (order[orderIdx]/Math.abs(order[orderIdx]))*(Math.abs(order[orderIdx])-opts.defOrder-1);

		if (orderIdx > 0) {
			var sleepy = Math.abs(lastOrder-Math.abs(normalisedOrder));
			if (sleepy == 0) { sleepy = 20; }
			console.log("Norm " + normalisedOrder + "  Sleep " + sleepy);
			...
			for (var t = tFirst; t != tStop; t+=tIncr) {
				...
				if (opts.createWindows && orderIdx == 0) {
					...
					for (var tabNum = 0; tabNum < win.tabs.length; tabNum++) {
						...
						console.log("196 [" + normalisedOrder + "] w " + w + " tab " + t + "  want "+tabName + "  got "+tabNameVal);
				...
				for (var p = pFirst; p != pStop; p+=pIncr) {
					console.log("248 [" + normalisedOrder + "] w " + w + "  tab " + t + "  p2 " + p + "  sess " + sessionIndex + " real opts: " + JSON.stringify(panesList[p]));
					...
					if (opts.takeDown && order[orderIdx] == -realOrder) {
						...
						if (panesList[p].debug) {
							sendLine(win, t, sessionIndex, cmdPrefix + "echo $(date) 'DOWN \"" + paneName +"\" [" + panesList[p].priority + "] \"" + panesList[p].stop_command + "\"'");
						} else {
					...
					if (opts.bringUp && order[orderIdx] == realOrder) {
						for (var ev in panesList[p].env) {
							sendLine(win, t, sessionIndex, cmdPrefix + "export " + ev + `="` + panesList[p].env[ev] + "\"")
						}
						if (panesList[p].debug) {
							sendLine(win, t, sessionIndex, cmdPrefix + "echo $(date) 'UP   \"" + paneName + "\" [" + panesList[p].priority + "] \"" + panesList[p].start_command + "\"'");
						} else {
