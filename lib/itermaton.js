function getOpts(argv) {
	...
	if (opts.configFile == "") {
		logAt(1, opts, "Need config-file arg")
		return 3
	}
	return opts
}

function logAt(lev, opts) {
		...
		for (var t = 0; t < tabList.length; t++) {
			...
			for (var p = 0; p < panesList.length; p++) {
				...
				if (panesList[p].priority < -opts.defOrder || panesList[p].priority > opts.defOrder) {
					logAt(1, opts, "Order value " + panesList[p].priority + " not in expected [-" + opts.defOrder + "," + opts.defOrder + "] range")
					return 4
				}
	...
	for (var orderIdx = 0; orderIdx < order.length; orderIdx++) {
		...
		for (var w = winFirst; w != winStop; w+=winIncr) {
			...
			if (winName === undefined) {
				logAt(1, opts, "Cannot find name for window with index: " + w)
				return 2
			}
			...
			if (opts.createWindows && orderIdx == 0) {
				...
				if (win === undefined) {
					logAt(1, opts, "Cannot find window " + winName)
					return 2
				}
			...
			for (var t = tFirst; t != tStop; t+=tIncr) {
				...
				if (tabName === undefined) {
					logAt(1, opts, "Cannot find name for tab with index: " + t + " within window: " + tabName)
					return 2
				}
				...
				if (opts.createWindows && orderIdx == 0) {
					...
					for (var tabNum = 0; tabNum < win.tabs.length; tabNum++) {
						...
						logAt(1, opts, "230 [" + normalisedOrder + "] w " + w + " tab " + t + "  want "+tabName + "  got "+tabNameVal);
				...
				for (var p = pFirst; p != pStop; p+=pIncr) {
					logAt(1, opts, "283 [" + normalisedOrder + "] w " + w + "  tab " + t + "  p2 " + p + "  sess " + sessionIndex + " real opts: " + JSON.stringify(panesList[p]));
					var paneName = panesList[p].name;
					if (paneName === undefined) {
						logAt(1, opts, "285 Cannot find name for pane with index: " + t + " within window: " + paneName + " in tab: " + tabName)
						return 2
					}
					...
					if (opts.createWindows && orderIdx == 0) {
						if (p > 0) {
							...
							if (newPane) {
								// logAt(1, opts, "win271 " + w + " p " + p + " tab " + t + " sess " + sessionIndex + ' new pane');
								// setBadge(win.tabs[t].sessions[sessionIndex], paneName)
							}
							sessionIndex++
						}

						// logAt(1, opts, "win277 " + w + " p " + p + " tab " + t + " sess " + sessionIndex);
						...
						for (sessionIndex = sessFirst; sessionIndex != sessStop; sessionIndex+=sessIncr) {
							logAt(1, opts, w+" "+t+" "+p+ " top="+sessionIndex + " len="+tab.sessions.length+"  "+paneName)
							// race condition xref_close_race
							var sess = tab.sessions[sessionIndex]
							if (sess===undefined) {
								logAt(1, opts, w+" "+t+" "+p+" sess="+sessionIndex + " undef sess for " + paneName)
							}
							var sessNameVal = getVar(sess, "sessName")
							if (sessNameVal === undefined){
								logAt(1, opts, w+" "+t+" "+p+" sess="+sessionIndex + " undef NAME for " + paneName)
							}
							if (sessNameVal == paneName) {
								gotPane = true
								break
							}
							logAt(1, opts, w+" "+t+" "+p+" sess="+sessionIndex + "... " + paneName + " != " + sessNameVal)
						}
						logAt(1, opts, w+" "+t+" "+p+ " post")
						if (!gotPane) {
					...
					logAt(1, opts, w+" "+t+" "+p+ " l=345  "+sessionIndex)

					var realOrder = opts.defOrder + panesList[p].priority + 1
					logAt(1, opts, w+" "+t+" "+p+ " l=348")
					...
					logAt(1, opts, w+" "+t+" "+p+ " l=360")

					if (opts.bringUp && order[orderIdx] == realOrder) {
						for (var cmdN in panesList[p].init_commands) {
							sendLine(win, t, sessionIndex, cmdPrefix + panesList[p].init_commands[cmdN])
						}
						for (var cmdN in panesList[p].late_init_commands) {
							sendLine(win, t, sessionIndex, cmdPrefix + panesList[p].late_init_commands[cmdN])
						}
						if (panesList[p].debug) {
					...
					logAt(1, opts, w+" "+t+" "+p+ " l=380")

				}
				logAt(1, opts, w+" "+t+" "+p+ " l=383")
